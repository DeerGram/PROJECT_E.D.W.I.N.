...
outputFolder = os.path.join(OUTPUT_DIRECTORY, os.path.splitext(os.path.basename(originalVideoFile))[0])
...
def trim_clip(inputSound):
    ...
    Args:
    inputSound (AudioSegment): The audio clip to be trimmed.

    Returns:
    AudioSegment: The trimmed audio clip.
    """
...
def insert_audio(canvas, audioToOverlay, startTimeMs):
    """
    Insert an audio clip into the canvas at a specific point.

    Args:
    canvas (AudioSegment): The canvas to insert the audio clip into.
    audioToOverlay (AudioSegment): The audio clip to be inserted.
    startTimeMs (int): The start time in milliseconds for the audio clip.

    Returns:
    AudioSegment: The canvas with the inserted audio clip.
    """
    canvasCopy = canvas
...
def create_canvas(canvasDuration, frame_rate=nativeSampleRate):
    """
    Create a silent audio canvas of a specific duration.

    Args:
    canvasDuration (int): The duration of the canvas in milliseconds.
    frame_rate (int): The frame rate of the canvas (default: nativeSampleRate).

    Returns:
    AudioSegment: The silent audio canvas.
    """
